...
for i in range(1, 11):
   ...
   # model = SVC(C=1.0, gamma=0.01, kernel='poly', probability=True) no ling
    ...
    SVM = '/export/b15/afavaro/Frontiers/submission/Classification_With_Feats_Selection/Cross_Val_Results_2/GITA/All_Tasks/AUROC/'
    ...
    #model = KNeighborsClassifier(metric='minkowski', n_neighbors= 17, weights= 'distance') no ling
   ...
   # model = GradientBoostingClassifier(learning_rate=0.001, max_depth=3, n_estimators=1000, subsample=1.0) #No ling
    grid_result = model.fit(X_train, y_train)
    grid_predictions = grid_result.predict_proba(X_test)
    grid_predictions = grid_predictions[:, 1]
